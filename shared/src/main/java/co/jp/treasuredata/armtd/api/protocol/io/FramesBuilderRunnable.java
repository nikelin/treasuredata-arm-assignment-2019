...
public class FramesBuilderRunnable implements Runnable {
  private static final Logger logger = LoggerFactory.getLogger(FramesBuilderRunnable.class);
  ...
  public void run() {
    ...
    while (!this.terminateSignal.get()) {
      try {
        ...
        PacketsBuilder builder =
            inFlightFrame.computeIfAbsent(socketChannel, (k) -> packetsBuilder);
