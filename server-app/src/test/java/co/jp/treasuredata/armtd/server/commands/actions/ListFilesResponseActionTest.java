...
public class ListFilesResponseActionTest {
    ...
    @Test
    public void testFilesListingForExistingDirectory() throws IOException, InterruptedException, ExecutionException {
        ...
        Assert.assertTrue("all files are present", Arrays.asList(filesList).containsAll(expectedFilesList));
    }
    ...
    public void testFilesListingForNonExistingDirectory() throws Throwable {
        try {
            int testToken = 10023;
            Request request = new Request(testToken, "test", new String[]{});
            ListFilesResponseAction action = new ListFilesResponseAction(new File("non-existing-directory"));
            CompletableFuture<List<Packet>> future = action.execute(request);
            future.get();
        } catch (Throwable e) {
            throw e.getCause();
