...
public class ServerIntegrationalTest {
    ...
    public void testSingleConnectionBasicRequest() throws Throwable {
        ...
        for ( int i = 0; i < 1000; i++ ) {
            results.addAll(
                executeConnection(i, 1, serverPort,
                    Collections.singletonList(128),
                    (idx) -> new Packet(idx, (checkPayload + " " + idx).getBytes()),
                    readSingleBuffer
                ).stream().map((v) -> new String(v.getData())).collect(Collectors.toList())
            );
        }
        ...
        for (int i = 1; i < 1001; i++) {
            assertTrue("results contain a correct value for #" + i, results.contains(checkPayload + " " + i));
        }
    ...
    public void testMultipleConnectionBasicRequest() throws Throwable {
        ...
        for (int connection = 1; connection < 101; connection++) {
            Future<List<String>> future = service.submit(() -> {
                try {
                    return executeConnection(connectionId.getAndIncrement(), 100, serverPort,
                            Collections.singletonList(128),
    ...
    private List<Packet> executeConnection(int connectionId,
                                   int requestsCount,
                                   int serverPort,
                                   List<Integer> bufferSizes,
                                   Function<Integer, Packet> packetBuilder,
                                   BiFunction<SocketChannel, List<Integer>, List<Packet>> readPackets) throws IOException {
        SocketChannel socket = SocketChannel.open();
    ...
    private final BiFunction<SocketChannel, List<Integer>, List<Packet>> readTwoBuffers = (socket, bufferSizes) -> {
        try {
            ...
            return Arrays.asList(
                new Packet(token, data),
                new Packet(dataToken, bodyData)
            );
        } catch (Throwable e) {
    ...
    private final BiFunction<SocketChannel, List<Integer>, List<Packet>> readSingleBuffer = (socket, bufferSizes) -> {
        try {
            ByteBuffer buffer = ByteBuffer.allocate(bufferSizes.get(0));
            socket.read(buffer);
            buffer.flip();
