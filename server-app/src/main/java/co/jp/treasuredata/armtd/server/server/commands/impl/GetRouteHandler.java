...
@DiscoverableHandler(
    mappings = {"get"},
    description = "Command to fetch a given file from the root directory specified on startup")
public class GetRouteHandler implements RouteHandler {
  ...
  public List<ResponseAction> handleRequest(Server server, Request request)
    ...
    String requestedFilePath =
        request.getArguments().length == 0 ? null : request.getArguments()[0];
    File file = new File(config.getInputDirectoryPath(), requestedFilePath);

    final List<ResponseAction> result;
    if (file.exists()) {
      result = Arrays.asList(new TextResponseAction("ok"), new FileResponseAction(file));
    } else {
      result =
          Collections.singletonList(
              new ErrorResponseAction(
                  ErrorResponseAction.FILE_NOT_FOUND, requestedFilePath + " not found"));
    }
