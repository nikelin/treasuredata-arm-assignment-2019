...
public class App {
    ...
    public static void main(String[] args) {
        ...
        if (settingsStream == null) {
            logger.error("ERROR: Application properties is not present on the classpath (classpath:application.properties)");
            exit(-1);
            return;
        }
        ...
        try {
            ...
            logger.error("ERROR: Unable to load application properties file (classpath:application.properties)");
            exit(-1);
            return;
        }
        ...
        final Optional<PacketHandlerProvider> packetHandlerOpt = resolveServiceProvider(PacketHandlerProvider.class);
        ...
        final Server server;
        try {
            server = serverProviderOpt.get().provide(packetHandler, new DiscoveryRoutingRouteHandler(
                    new PropertiesBasedHandlersDiscoverer(serverConfig, properties).resolveHandlers()));
