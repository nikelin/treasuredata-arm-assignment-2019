...
public class PropertiesBasedHandlersDiscoverer implements HandlersDiscoverer {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(PropertiesBasedHandlersDiscoverer.class);
  ...
  public PropertiesBasedHandlersDiscoverer(
      ServerConfig config, Properties properties, ClassLoader classLoader) {
    ...
    while (keys.hasMoreElements()) {
      ...
      try {
        ...
        throw new DiscovererException(
            "Handler class " + className + " is not available in the classpath", e);
      }

      if (!RouteHandler.class.isAssignableFrom(handlerClazz)) {
        throw new DiscovererException(
            "Handler class " + className + " must be an instance of " + RouteHandler.class);
      }

      DiscoverableHandler annotation = handlerClazz.getAnnotation(DiscoverableHandler.class);
      if (annotation == null) {
        throw new DiscovererException(
            "Handler class " + className + " must be annotated with " + DiscoverableHandler.class);
      }
      ...
      try {
        ...
        if (constructors.length == 0) {
          ...
          if (targetConstructor == null) {
            throw new DiscovererException(
                "At least a single empty public constructor expected on type " + className);
        ...
        throw new DiscovererException(
            "Failure during the instantiation of the handler class " + className, e);
      }

      if (handler == null) continue;

      logger.info(
          "New commands handler registered - "
              + className
              + " (mappings: "
