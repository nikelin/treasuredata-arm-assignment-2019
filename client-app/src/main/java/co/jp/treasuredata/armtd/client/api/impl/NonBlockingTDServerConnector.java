...
public class NonBlockingTDServerConnector implements TDServerConnector {
  ...
  public NonBlockingTDServerConnector(
  ...
  private void socketConnect() throws IOException {
    ...
    socketChannel.connect(
        config.getServerHost() != null
            ? new InetSocketAddress(config.getServerHost(), config.getServerPort())
  ...
  public void connect() throws IOException, InterruptedException, ExecutionException {
    if (!this.isReconnecting.compareAndSet(false, true)) {
      consoleOut.println(
          "[status] Connection request declined - there is another connection request in progress already.");
      return;
    }

    int attempts = 0;
    int maxAttempts =
        config.getMaxReconnectAttempts() == null
                ...
                if (!selectedKey.isValid()) {
                  ...
                  this.executorService.execute(
                      new FramesBuilderRunnable(
                          1,
                          new StandardPacketsBuilder(Integer.MAX_VALUE),
                          readBuffers,
                          packetsQueue,
                          isTerminated));
  ...
  private void handleBroadcastPacket(Packet packet) {
    if (this.broadcastPacketHandler == null) {
      if (config.getVerbose()) {
        consoleOut.println(
            "[warning] No broadcast handler defined - ignoring received broadcast packet");
      }
    } else {
  ...
  private CompletableFuture<Response> sendCmd(String commandText, int expectedResponses) {
    ...
    this.clientRequests.add(
        new ClientRequest(
            new Packet(requestTokenSeq.getAndIncrement(), commandText.getBytes()),
            promise,
            expectedResponses));
    return promise;
  }
  ...
  public CompletableFuture<List<String>> listFiles() {
    return CompletableFuture.supplyAsync(System::currentTimeMillis)
        .thenCompose(
            (startedAt) ->
                sendCmd("index", 1)
                    .thenApply((data) -> Arrays.asList(new String(data.data).trim().split("\n\r")))
                    .thenApply(
                        (list) -> {
                          if (config.getVerbose()) {
                            consoleOut.println(
  ...
  public CompletableFuture<String> fetchFile(String name) {
    return sendCmd("get " + name, 2).thenApply((v) -> new String(v.data).trim());
  }

  @Override
  public CompletableFuture<Void> quit() {
    return sendCmd("quit", 1).thenAccept((v) -> {});
  }
